...
```{r}
clist_ipssm_mat_prop2 <- prop.table(table(clinical_low$clust, clinical_low$IPSSM), margin = 1) %>%
    as.matrix()*100 
...
survfit(formula = Surv(OS_YEARS,OS_STATUS) ~ IPSSM_col, main_IPSSM) %>%
    ggsurvplot(data = main_IPSSM, surv.median.line = "hv", 
               palette = ipssm_cols, facet.by = "clust")
```

```{r}
os_cl_res <- lapply(main_clusts, function(cl){
    ipssm_test <- lapply(IPSSM_cats, function(cat){
    tmp <- subset(main_IPSSM, clust == cl) %>%
...
```{r}
main_treat <- filter(main_low, IPSSM %in% c("Moderate-High", "High", "Very-High") & clust != "Y-") %>%
...
asxl1_test <- lapply(main_clusts, function(cl){
    tmp <- mutate(main_low,
             cluster = relevel(clust, cl))
    os <- summary(coxph(Surv(OS_YEARS,OS_STATUS) ~ cluster*ASXL1 + AGE + SEX + hma + transplant, tmp))
     coefs <- os$coefficients
     coef_df <- as_tibble(coefs[, c(1, 5)]) %>%
       mutate(Ref_cluster = cl, 
              Comp_clust = gsub("cluster", "", rownames(coefs)),
...
dnmt3a_test <- lapply(main_clusts, function(cl){
    tmp <- mutate(main_low,
             cluster = relevel(clust, cl))
    os <- summary(coxph(Surv(OS_YEARS,OS_STATUS) ~ cluster*DNMT3A + AGE + SEX + hma + transplant, tmp))
     coefs <- os$coefficients
     coef_df <- as_tibble(coefs[, c(1, 5)]) %>%
       mutate(Ref_cluster = cl, 
              Comp_clust = gsub("cluster", "", rownames(coefs)),
...
srsf2_test <- lapply(main_clusts, function(cl){
    tmp <- mutate(main_low, 
             cluster = relevel(clust, cl))
    os <- summary(coxph(Surv(OS_YEARS,OS_STATUS) ~ cluster*SRSF2 + AGE + SEX + hma + transplant, tmp))
     coefs <- os$coefficients
     coef_df <- as_tibble(coefs[, c(1, 5)]) %>%
       mutate(Ref_cluster = cl, 
              Comp_clust = gsub("cluster", "", rownames(coefs)),

...
```{r}
load("../results/clustering/high_blast_filter_pc_clustering.Rdata")
load("../results/clustering/high_blast_filter_pc_clustering_raw.Rdata")
```

```{r}
kar_events <- c("delY", "del11q", "del20q", "del7q",  "plus8",  "del7", "del5q", "del12p")
sel_muts <- c("TET2", "ASXL1", "SRSF2", "DNMT3A", "RUNX1", "TP53", "NRAS", "DDX41", 
              "MLL_PTD", "IDH1", "CEBPA", "TP53mono", 
...
mut_hr_col %>%
  filter(N > 10) %>%
    filter(Gene %in% c("ASXL1", "DNMT3A", "RUNX1", "SF3B1", "SRSF2", "STAG2")) %>%
    ggplot(aes(x = Cluster, y = HR, fill = Color)) +
        geom_bar(stat = "identity") +
        theme_bw() +
        scale_y_continuous(transform = "log2") +
        facet_wrap(~ Gene, scales = "free_x") +
        scale_fill_manual(name = "", values = c("green", "darkgreen", "darkred", "red")) +
    theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```

...
gesmd_low <- filter(gesmd_data, consensus %in% c("Low blasts", "MDS-IB1", "MDS-IB2"))

```

```{r}
kar_events <- c("delY", "del11q", "del20q", "del7q",  "plus8",  "del7", "del5q")
sel_muts <- c("TET2", "ASXL1", "SRSF2", "DNMT3A", "RUNX1", "TP53", "NRAS", 
              "IDH1", "CEBPA", 
...
mut_hr_clust2 <- lapply(test_muts, function(gene){
    lapply(main_clusts, function(cl){
        ...
        hr <- exp(coef[1, 1])
        pval <- coef[1, 5]
        freq <- mean(main_low[[gene]][main_low$clust == cl], na.rm = TRUE)
        N <- sum(main_low[[gene]][main_low$clust == cl], na.rm = TRUE)

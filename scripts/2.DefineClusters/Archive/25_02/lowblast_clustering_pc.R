...
png("figures/lowblast_pc_cluster/comb_kmeans_silhouette.png")
plot(2:20, sil_scores, type="b", pch = 19, frame = FALSE,
     xlab = "Number of clusters K",
     ylab = "Silhouette Score",
     main = "Combined clustering")
dev.off()
clust_comb <- kmeans(comb_pc$x[, 1:8], centers = 7, nstart = 1000)$cluster
...
png("figures/lowblast_pc_cluster/lowblasts_classification_tree_ori.png", width = 1000, height = 650)
plot(tree_mod)
...
png("figures/lowblast_pc_cluster/lowblasts_classification_tree.png", width = 1000, height = 650)
plot(tree_mod)
text(tree_mod, use.n = TRUE)
dev.off()

## Relabel clusters and simplify classification
labels <- c("Low WBC", "8+", "Y-", "TET2 bi", "del20q", "TET2 mono", "High WBC")
tree_data_slim <- tree_dataset %>% 
    mutate(clust = factor(labels[as.numeric(clust)], 
           levels = c("Low WBC", "High WBC", "TET2 mono", "TET2 bi", "Y-", "8+", "del20q"))) %>%
    select("TET2", "WBC", "del20q", "plus8", "delY", "TET2other", "ANC", clust)

tree_labels <- rpart(clust ~ . , data = tree_data_slim, method = "class", 
                     control = rpart.control(cp = 0.015))

png("figures/lowblast_pc_cluster/lowblasts_classification_tree_labels.png", width = 1000, height = 650)
plot(tree_labels)
text(tree_labels, use.n = TRUE)
dev.off()
table(new = predict(tree_labels, tree_data_slim, type = "class"), ori = tree_data_slim$clust)

save(sil_scores, tree_labels, clin_pc, kar_pca, mut_pca, comb_pc, file = "results/clustering/lowblast_pc_clustering_raw.Rdata")
...
save(clinical_low, file = "results/clustering/lowblast_pc_clustering.Rdata")

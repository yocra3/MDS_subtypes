...
from NNmodel import survivalNN
...
net = survivalNN(n_feats, hidden_units).to(device)
...
_ = log.plot()
plt.savefig('figures/NN_models/boolean_NNfull_training.png', dpi=300, format='png')
...
model.save_net("results/mutations/fullNN_boolean_model.pt")
model.save_model_weights("results/mutations/fullNN_boolean_weights.pt")
...
activation = {}
hook = model.net.NN.register_forward_hook(get_activation(activation))
model.net(x_test[0].to(device), x_test[1].to(device))
hook.remove()

bol_nn_np = activation['output'].cpu().numpy()
bol_scores_df = pd.DataFrame(bol_nn_np)
bol_scores_df['ID'] = df_test['ID']
bol_scores_df.to_csv("results/mutations/boolean_scores_full.tsv", sep = "\t", index = False)
...
## Define model
net2 = survivalNN(n_feats, hidden_units).to(device)
...
_ = log_vaf.plot()
plt.savefig('figures/NN_models/vaf_NNfull_training.png', dpi=300, format='png')

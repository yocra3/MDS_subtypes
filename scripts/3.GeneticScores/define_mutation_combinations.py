...
class MutationCombinationsPipeline:
    ...
    def run_pipeline(self) -> Dict[str, Any]:
        ...
        try:
            ...
            graphs = self.generate_graph_data_individual(df_processed, self.config['variable_processing'],
    ...
    def generate_graph_data_individual(self, df_processed: pd.DataFrame, metadata: Dict[str, Any],
        ...
        for patient_idx in range(len(df_processed)):
            ...
            base = self.data_generators._create_hetero_graph_for_patient(df_processed, 
                patient_idx, gene_features, [], df_mutations)
            patient_graphs['base'] = base
            if len(patient_mutations) > 0:
                ...
                for combo in gene_combos:
                    ...
                    df_mut_filtered = patient_mutations[patient_mutations['GENE'].isin(combo)]
                    if combo_key not in patient_graphs:
                        patient_graphs[combo_key] = self.data_generators._create_hetero_graph_for_patient(
                            df_processed, patient_idx, gene_features, selected_genes, df_mut_filtered
                        )
            graph_list[patient_id] = patient_graphs
            print(f"Patient {patient_id} processed with {len(patient_graphs)} graphs.")
        return graph_list
...
Ejemplos de uso:
  ...
  python prepare_data_unified.py --config configs/full_vars.yaml --output results/full_analysis
    ...
    try:
        # Crear pipeline
        pipeline = MutationCombinationsPipeline(args.config, args.output)
